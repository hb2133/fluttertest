# ARCHITECTURE RULES (Panel Layer Runtime, Flutter Profile) V1

## 1) 문서 역할
- 이 문서는 Flutter/Dart 프로젝트에서 `LayeredPanel` runtime 규칙을 정의하는 문서다.
- 이 문서는 `ARCHITECTURE_RULES_PANEL_SECTION_FLUTTER_V1.md`의 `LayeredPanel` 개념을 실제 runtime 계약으로 확장한다.
...
- tooltip, hover hint, dropdown 같은 초경량 부유 UI는 반드시 이 시스템을 사용할 필요는 없다.
...
## 6) PanelLayerHost 규칙
- 경로: `lib/app/panel_layer/panel_layer_host.dart`
- 역할:
...
- dismiss, ESC, barrier 입력은 기본적으로 top panel 기준으로 처리한다.
- 별도 `panel_layer_state.dart`, `panel_layer_types.dart`, `panel_layer_registry.dart` 파일은 필수가 아니다.
- 기본 문서에서는 현재 `BasePanel Controller` 내부 구조체로 시작해도 된다.

### 7-1) Layered Stack 예시 스니펫
```dart
enum LayeredPanelPhase
{
  Entering,
  Visible,
  Closing,
...
class LayeredPanelEntry<TPayload, TResult>
{
  final String PanelId;
  final LayeredPanelPhase Phase;
  final TPayload? Payload;
  final bool Dismissible;
  final Object? AnchorMetadata;
  final void Function(TResult Result)? OnComplete;
  final void Function(LayeredCloseReason Reason)? OnRequestClose;

  const LayeredPanelEntry({
...
- Flutter에서는 `FocusScope`, `Shortcuts`, `Actions`를 통해 키보드 우선권을 연결하는 편을 권장한다.

## 13) Barrier / Dismiss 정책
- `LayeredPanel`은 dismiss 가능 여부를 가질 수 있다.
- dismiss trigger는 최소 `barrier tap`, `Escape`, `explicit close action`을 포함할 수 있다.
- top panel만 dismiss 대상이 된다.
- dismiss 불가 panel은 barrier/ESC에 반응하지 않는다.

## 14) Anchor Popup 대응 기준
- anchor 기반 popup panel은 open 시점에 `anchor metadata`를 받을 수 있다.
- Flutter 기준 anchor metadata는 `LayerLink`, `Rect`, `Offset` 같은 위치 기준 데이터로 해석하는 편을 권장한다.
- 실제 위치 계산, clipping 보정, viewport 내 재배치는 `PanelLayerHost`가 담당한다.
- anchor가 무효가 되면 popup panel은 닫거나 fallback 위치 정책을 따른다.
- 위치 기준이 실시간으로 변할 경우 `RenderBox.localToGlobal`, `CompositedTransformTarget`, `CompositedTransformFollower` 계열로 재계산하는 편을 권장한다.
...
- 다만 barrier, focus trap, completion, 재사용성, 독립 payload가 커지는 순간 `LayeredPanel`로 승격한다.

## 22) 생성 시 금지 패턴
- 전역 자유 overlay stack을 기본 패턴처럼 사용하는 구조
- `PanelLayerHost` 없이 `BasePanel` Widget tree에 `LayeredPanel`을 무질서한 조건문으로 흩뿌리는 구조
