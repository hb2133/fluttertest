# Flutter Panel-Section Review Scoring Guide V1

## Overview
Flutter/Dart 프로젝트의 코드 변경사항을 리뷰하고, 현재 `Panel-Section-Controller` 지침 문서 기준으로 점수화 평가를 수행한다.

기준 문서:
- `ARCHITECTURE_RULES_PANEL_SECTION_CORE_V1.md`
- `ARCHITECTURE_RULES_PANEL_SECTION_FLUTTER_V1.md`
- `ARCHITECTURE_RULES_PANEL_LAYER_FLUTTER_V1.md`
- 필요 시 프로젝트별 플랫폼 규칙
...
```text
/flutter-review <commit-hash>
/flutter-review <branch-name>
/flutter-review <diff-or-files>
```

---

## 리뷰어 역할
당신은 시니어 Flutter 엔지니어이자 구조 리뷰어다.
목표는 단순 스타일 지적이 아니라, 현재 Flutter panel-section 지침 문서 기준으로 구조 일치도, 유지보수성, 리팩토링 필요도를 채점하는 것이다.
...
### P0-2. 구조 위반
- `BasePanel` root 없이 `main.dart` 또는 router가 화면을 직접 조립
- `Section`이 `BasePanel` 없이 직접 최상위 화면처럼 조립됨
- `LayeredPanel`이 route 대상으로 직접 사용됨
- `PanelLayerHost` 없이 overlay를 무질서한 조건문으로 직접 뿌림
- Widget tree 또는 `build(...)` 구현이 `controller/`, `actions/`, `state`, `managers`, `core`에 들어감
...
- `lib/core/localization/`를 우회하는 UI 라벨, 버튼 텍스트, 에러 문구 추가

### P0-5. 검증 실패
- `dart format --set-exit-if-changed .` 실패
- `flutter analyze` 실패
- import/export 미완결 또는 타입 오류로 코드가 깨짐
...
- `Section` 안에 문자열 조합, 조건 분기, 데이터 가공, 저장 규칙이 직접 들어감
- 메뉴/탭/옵션 배열 등의 하드코딩 구조 데이터가 Widget tree에 고정됨

### P1-2. Controller 오염
- `Controller`가 `core/infra`, 외부 API, 파일시스템을 직접 호출
- `Controller`가 `BuildContext` 또는 Widget 생명주기에 과도하게 의존
- layered stack 갱신, navigation, action orchestration이 한 파일에 과밀함

### P1-3. Action / State 정리 부족
- `actions/*.dart`가 validation, manager 호출, 예외 흐름을 안정적으로 분리하지 못함
- `state.dart`, `types.dart`에 타입, 초기 상태, selector, 파생값이 정리되지 않음
- `dynamic` 남용
- dispose가 필요한 객체 정리가 누락됨

### P1-4. App-Level 경계 오염
- `main.dart` 또는 router가 진입점 역할을 넘어서 복잡한 UI/비즈니스 로직을 가짐
- `app_shell.dart`가 panel 비즈니스 로직이나 layered stack 원본 상태를 소유
- `app_navigator.dart`가 panel 내부 의미를 과도하게 해석

### P1-5. Layer 규칙 느슨함
- `panel_layer_host.dart`가 렌더링 host를 넘어서 도메인 의미를 소유
- `LayeredPanel` payload/result 계약이 모호함
- `OnComplete`, `OnRequestClose` 대신 임의 패턴이 난립함

### P1-6. Core / Design / Localization 위반
- panel 전용 책임을 `lib/core/config|infra|localization|services`로 밀어 넣음
- 디자인 토큰 대신 매직 넘버 사용
...
- `lib/core/config|infra|localization|services` 역할 준수
- 문자열 하드코딩 금지 준수
- `lib/design/global_design.dart` 토큰 사용 여부
...
```markdown
## Flutter Panel-Section Review Score
...
| Panel Structure Quality | 17 | 20 | Section 정리는 양호하나 파생값 일부가 Widget에 남음 |
| Layer / Navigation Discipline | 16 | 20 | layered completion 흐름이 다소 모호 |
| Core / Localization / Design Discipline | 14 | 15 | 대체로 양호 |
| Project Conventions | 4 | 5 | brace style 1건 불일치 |
| Validation Readiness | 8 | 10 | analyze와 test 기준 양호 |

### P0 Blockers
- 없음

### P1 Issues
1. `lib/panels/base/.../sections/...` 에 사용자 표시 문자열 하드코딩 존재
...
- 리뷰는 반드시 현재 Flutter panel-section 지침 문서 기준으로 설명한다.
- 점수는 항목별 이유와 함께 제시한다.
- P0와 P1은 총점보다 우선한다.
